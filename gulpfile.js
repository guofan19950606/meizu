...
gulp.task("copy-html",async ()=>{
	...
	gulp.src("*.html")
	.pipe(gulp.dest("D:\\phpStudy\\WWW\\meizupro"));
});


gulp.task("copy-img",async ()=>{
	gulp.src("img/*.{jpg,png,web,webp}").pipe(gulp.dest("D:\\phpStudy\\WWW\\meizupro\\img"));
});

gulp.task("copy-js",async ()=>{
	gulp.src("js/*")
	.pipe(gulp.dest("D:\\phpStudy\\WWW\\meizupro\\js"));
});
//监听并更改所有css文件
gulp.task("copy-css",async ()=>{
	gulp.src("css/*")
	.pipe(gulp.dest("D:\\phpStudy\\WWW\\meizupro\\css"));
})
